...
<body>
	<center>
		<font size="6" color="red">更&nbsp;&nbsp;&nbsp;&nbsp;新</font> <br>
		<%
			Admin admin = (Admin) request.getAttribute("admin");
		%>
		<form action='UpdateAdminServlet' method='post' name="myform">
			<input type = "hidden" name="id" value="<%=admin.getId()%>">
			<input type = "hidden" name="preUsername" id="preUsername" value='<%=admin.getUsername() != null? admin.getUsername(): ""%>'>
			<table border="1px" width="90%" bgcolor="E7F1FF" bordercolor="BFCCDA"
				...
				<tr>
					...
					<td height=50 width="18%" valign="top">
						...
						value='<%=admin.getUsername() != null? admin.getUsername(): ""%>' /> <span id="checkStr"></span></td>
				</tr>
				<tr>
					<td height=50 width="12%" align="right">密码：</td>
					<td height=50 width="18%" valign="top">
						<input type='password' name='password' size="18" onBlur="checkNull2()" value='<%=admin.getPassword() != null? admin.getPassword(): ""%>'/>
				...
				<tr>
					...
					<td height=50 width="18%" valign="top">
						<input type='password' name='password2' size="18" onBlur="checkNull2()" value='<%=admin.getPassword() != null? admin.getPassword(): ""%>'/>
				...
				<tr>
					...
					<td height=50 width="18%" valign="top">
						<% 
							if (admin.getSex() != null && admin.getSex().equals("女")) {
						%>	
				...
				<tr>
					...
					<td height=50 width="18%" valign="top">
						<input type='text' name='age' value='<%=admin.getAge() != null? admin.getAge(): ""%>' onblur="checkAge();"/>
				...
				<tr>
					...
					<td height=50 width="18%" valign="top">
						<% 
							if (admin.getAuth() != null && admin.getAuth() == 0) {
						%>	
						<input type='radio' name='auth' value="2" /> 普通员工
						<input type='radio' name='auth' value="1" /> 普通管理员
						<input type='radio' name='auth' value="0" checked/> 超级管理员
						<%
							} else if (admin.getAuth() != null && admin.getAuth() == 1){
						%>
						<input type='radio' name='auth' value="2" /> 普通员工
						<input type='radio' name='auth' value="1" checked/> 普通管理员
						<input type='radio' name='auth' value="0" /> 超级管理员
						<%
							} else if (admin.getAuth() != null && admin.getAuth() == 2){
						%>
